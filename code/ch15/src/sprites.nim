...
proc initStaticSprite*(atlas: Atlas, name: string,
                       scale: float32 = 4): AnimSprite =
  ## A one-frame "animation" for things that don't animate (swords,
  ## skulls), so everything drawn is the same component type.
  AnimSprite(anim: name, frames: @[atlas.rect(name)],
             secsPerFrame: 1, scale: scale)
...
proc srcRect*(s: AnimSprite): Rectangle =
  ## The current frame's atlas region, unflipped: where on the atlas
  ## this sprite samples right now (the outline shader needs to know).
  s.frames[s.index]
...
proc draw*(s: AnimSprite, atlas: Atlas, pos: Vector2,
  ...
  ## no second set of art required. The tint is how hurt-flashes reach
  ## the sprite without the sprite knowing about combat.
  ...
  drawTexture(atlas.texture, src, dest, Vector2(x: 0, y: 0), 0, tint)
