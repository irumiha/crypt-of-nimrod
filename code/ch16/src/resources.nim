...
##
## All of the game's art lives in one image (the atlas). This module
...
proc loadAtlas*(imagePath, indexPath: string): Atlas =
  ...
  ## The texture is freed automatically when the Atlas value dies
  ## (naylib types carry destructors); nobody calls unloadTexture.
  result.texture = loadTexture(imagePath)
  for line in readFile(indexPath).splitLines:
    # Each line: name x y width height. Short or empty lines (like the
    # trailing newline) fail the length check and are skipped.
...
proc frames*(atlas: Atlas, name: string): seq[Rectangle] =
  ...
  ## following the art pack's frame-naming convention.
  var i = 0
