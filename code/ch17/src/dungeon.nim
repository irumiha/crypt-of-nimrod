...
type
  ...
  Dungeon* = object
    ...
    stairsRoom*: int   # stairs down (or the throne, on the last floor)
    sealed: seq[tuple[x, y: int32]]
    dissolved: seq[tuple[x, y: int32]]  # where seals were, for relock
...
proc generate*(seed: int64, floorNum: int, final = false): Dungeon =
  ...
  ## On the final floor the deepest room holds no stairs; it is the
  ## throne room, and what waits there is the caller's business.
  ...
  # The stairs themselves, center of their room. The last floor gets
  # none: the descent ends here.
  if not final:
...
proc unlock*(d: var Dungeon) =
  ## The key dissolves every sealed tile into ordinary floor. The
  ## spots are remembered, because on the last floor the seals have
  ## one more entrance to make (relock).
  for (x, y) in d.sealed:
    d.map.setTile(x, y, tkFloor)
    d.dissolved.add((x, y))
  d.sealed.setLen(0)

proc relock*(d: var Dungeon) =
  ## The seals return: every dissolved door tile turns back to
  ## tkSealed. The boss fight's lock-in. Undone by unlock, like any
  ## other seal.
  for (x, y) in d.dissolved:
    d.map.setTile(x, y, tkSealed)
    d.sealed.add((x, y))
  d.dissolved.setLen(0)
...
proc insideRoom*(d: Dungeon, room: int, pos: Vector2,
                 marginTiles = 2): bool =
  ## Whether a world position is well inside a room's interior, at
  ## least marginTiles from its walls. The boss lock-in uses this so
  ## the seals never close on top of someone still in the doorway.
  let r = d.rooms[room]
  let left = float32((r.gx*roomW + marginTiles)*tileSize)
  let top = float32((r.gy*roomH + marginTiles)*tileSize)
  let right = float32(((r.gx + 1)*roomW - marginTiles)*tileSize)
  let bottom = float32(((r.gy + 1)*roomH - marginTiles)*tileSize)
  pos.x >= left and pos.x < right and pos.y >= top and pos.y < bottom
