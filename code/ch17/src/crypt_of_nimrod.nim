## Chapter 17: shipping it. The game is done; this chapter makes it
## portable. One structural change for the web build: the frame is a
## proc and the game's state is an object, because in a browser the
## event loop owns the program and main() has to hand over the wheel.
## Everything else ships as-is, on three desktop platforms and the web.
...
type
  Game = object
    ## Everything the running game owns: resources, phase, and the
    ## current run. On desktop it lives on main's stack, same as the
    ## loose vars it replaced. The web build is why it exists at all:
    ## there, main() returns while the game keeps running, so the
    ## state has to live somewhere that outlasts it.
...
proc initGame(): Game =
  ## Loads every resource and sets the opening scene. Call only after
  ## initWindow; everything here talks to the GPU or the audio device.
  result.canvas = loadCanvas(screenWidth, screenHeight)
  result.atlas = loadAtlas(
    atlasDir & "0x72_DungeonTilesetII_v1.7.png",
    atlasDir & "tile_list_v1.7")
  result.skin = makeSkin(result.atlas)
  result.fx = loadFx(result.atlas, screenWidth, screenHeight)
  result.bank = loadAudioBank()
  result.bank.startMusic()
  result.phase = gpMenu
  result.cam = makeCamera(Vector2(x: screenWidth, y: screenHeight))
  result.dbg = initDebug()

proc frame(g: var Game) =
  ## One whole frame: update by phase, draw the logical canvas, blit.
  ## Identical on every platform; who calls it is the platform
  ## difference. The templates below are zero-cost aliases, so the
  ## body reads exactly as it did when these were main's locals.
  template canvas: untyped = g.canvas
  template atlas: untyped = g.atlas
  template skin: untyped = g.skin
  template fx: untyped = g.fx
  template bank: untyped = g.bank
  template phase: untyped = g.phase
  template crtOn: untyped = g.crtOn
  template run: untyped = g.run
  template dust: untyped = g.dust
  template shake: untyped = g.shake
  template hitstop: untyped = g.hitstop
  template cam: untyped = g.cam
  template menuTime: untyped = g.menuTime
  template dbg: untyped = g.dbg
...
when defined(emscripten):
  # The browser owns the loop. A blocking `while` would freeze the
  # tab: JavaScript's event loop has to breathe between frames, so
  # emscripten calls our frame as a callback instead. The callback
  # can't capture main's stack frame, which is why the state lives
  # here, at module level, on this platform only.
  var game: Game
  proc webFrame() {.cdecl.} =
    frame(game)
...
proc main =
  ...
  when defined(emscripten):
    game = initGame()
    # fps 0 means "let the browser pace us" (requestAnimationFrame,
    # typically the display's refresh rate); the final 1 tells
    # emscripten to unwind main's stack but keep the page running.
    # Nothing after this line executes: the browser has the wheel.
    emscriptenSetMainLoop(webFrame, 0, 1)
  else:
    setTargetFPS(60)
    block:
      var game = initGame()
      while not windowShouldClose():
        frame(game)
    # The game (and its GPU/audio resources) died at the end of the
    # block above, in that order on purpose: destructors need the
    # window and audio device to still exist (the Chapter 12 lesson).
    closeAudioDevice()
    closeWindow()
