...
type
  ...
  PickupKind* = enum
    pkCoin, pkKey, pkHeart, pkMaxHp, pkPower, pkCrown

  BossPhase* = enum
    bpStalk, bpEnrage

  Boss* = object
    ## The boss's second brain, layered over the ordinary Ai chase.
    ## One phase flip at half health; while enraged, a timer meters
    ## out calls for help.
    phase*: BossPhase
    minionTimer*: float32
  ...
  DamageEvent* = tuple[pos: Vector2, amount: int32]
  ...
  World* = object
    ...
    floatTexts*: seq[string]
    bosses*: seq[Boss]
    # Frame scratch, not a component: contact pairs found this frame,
    # rewritten by contactSystem and read by whoever cares. Damage
    # events work the same way (damageSystem writes, the UI reads).
    contacts*: seq[Contact]
    damageEvents*: seq[DamageEvent]
    hovered*: int32 = -1          # slot under the mouse cursor, or -1
...
proc dump*(w: World, e: Entity): string =
  ...
  if ckFloatText in m:
    result.add &"\n  text      \"{w.floatTexts[e.idx]}\""
  if ckBoss in m:
    result.add &"\n  boss      {w.bosses[e.idx].phase} " &
               &"(minion call in {w.bosses[e.idx].minionTimer:.1f}s)"
  if ckPlayer in m:
