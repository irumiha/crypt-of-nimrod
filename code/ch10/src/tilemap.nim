...
const
  ...
  sealTint = Color(r: 232, g: 193, b: 112, a: 255)

type
  TileKind* = enum
    tkVoid, tkFloor, tkWall, tkSealed, tkStairs
...
proc initTilemap*(width, height: int32): Tilemap =
  ## An all-void map of the given size, floor variants pre-rolled, for
  ## generators to carve into.
  result.width = width
  result.height = height
  for i in 0 ..< width*height:
    result.tiles.add(tkVoid)
    result.floorVariants.add(
      if rand(1.0) < 0.9: 1'i32 else: int32(rand(2..8)))

proc setTile*(m: var Tilemap, x, y: int32, kind: TileKind) =
...
proc makeSkin*(atlas: Atlas): TileSkin =
  ...
  result.stairs = atlas.rect("floor_stairs")
...
proc draw*(m: Tilemap, atlas: Atlas, skin: TileSkin) =
  ...
  for y in 0'i32 ..< m.height:
    for x in 0'i32 ..< m.width:
      ...
      of tkSealed:
        # A locked door: the wall texture, dipped in gold. The crown's
        # magic, or an art budget of zero, depending who asks.
        drawTexture(atlas.texture, skin.wall, dest,
                    Vector2(x: 0, y: 0), 0, sealTint)
      of tkStairs:
        drawTexture(atlas.texture, skin.stairs, dest,
