## Chapter 13: the game gets manners and a voice. A state machine
## (menu / playing / paused / game over) wraps the whole thing, death
## finally means something, restarting is constructing a fresh Run
## value, and every sound in the crypt is synthesized from arithmetic
## at startup (see audio.nim).

import std/[math, options, random]
import raylib, raymath
import audio, camera, debug, dungeon, ecs, hud, input, loot, resources,
...
type
  GamePhase = enum
    gpMenu, gpPlaying, gpPaused, gpGameOver
...
proc newRun(atlas: Atlas): Run =
  ...
  result.crypt = generate(result.seed, 1)
...
proc main =
  ...
  while not windowShouldClose():
    ...
    if phase == gpMenu:
      ...
      drawCentered("WASD moves, SPACE swings, ESC pauses", 380, 10, Gray)
    else:
      beginMode2D(cam)
      run.crypt.map.draw(atlas, skin)
      run.world.drawSystem(atlas)
      ...
      if phase == gpPaused:
        ...
        drawCentered("floor " & $run.floorNum & "  |  " &
                     $run.kills & " kills  |  " &
                     $run.coins & " coins", 220, 20, LightGray)
