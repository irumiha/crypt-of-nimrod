# Chapter 13 — States and Sound

Game state at the end of this chapter: a real game loop around the
game. Title screen (the Chapter 1 crown, back home), pause on Esc, and
a game-over screen with the run's stats — death finally means
something, and restarting constructs a fresh `Run` value. Every sound
is synthesized from arithmetic at startup: nine effects and an
eight-bar A-minor theme, no audio files anywhere.

Build and run: `nimble run` — tests: `nimble test`

## Changes from ch12

| File | Status | Notes |
|------|--------|-------|
| `src/audio.nim` | new | the synthesizer: `tone` (shape/slide/envelope), `mix`, hand-built WAV headers, sfx bank, streamed music loop |
| `tests/taudio.nim` | new | synth invariants: length, envelope, determinism, saturating mix, WAV header |
| `src/crypt_of_nimrod.nim` | changed | `GamePhase` state machine (menu/playing/paused/game over), `Run` object (restart = new value), sfx wiring, `setExitKey` so Esc pauses instead of quitting |
| `src/input.nim` | changed | `aPause` action (Esc, P) |
| everything else | unchanged | |
