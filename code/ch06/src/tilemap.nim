...
type
  ...
  Tilemap* = object
    ...
    floorRects: seq[Rectangle]  # pre-rolled floor variant per cell
    wallRect: Rectangle

proc parseMap*(atlas: Atlas, ascii: string): Tilemap =
  ...
  for y in 0 ..< result.height:
    for x in 0 ..< result.width:
      ...
      result.floorRects.add(atlas.rect(name))
  result.wallRect = atlas.rect("wall_mid")
...
proc draw*(m: Tilemap, atlas: Atlas) =
  ...
  for y in 0'i32 ..< m.height:
    for x in 0'i32 ..< m.width:
      ...
      case m.tiles[i]
      of tkFloor:
        drawTexture(atlas.texture, m.floorRects[i], dest,
      ...
      of tkWall:
        ...
        # and draws darkened via the tint parameter. Two draw calls'
        # worth of logic instead of a full autotiling set, and the
        # rooms still read.
        let tint = if m.tileAt(x, y + 1) == tkFloor: White
                   else: wallTopTint
        drawTexture(atlas.texture, m.wallRect, dest,
