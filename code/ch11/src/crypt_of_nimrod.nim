## Chapter 11: the crypt pays out. The coin faucet is gone; loot now
## falls from dead enemies via a weighted drop table: coins, healing
## hearts, and two rare flasks that permanently raise max hp or sword
## damage. Balancing the economy is editing one table in loot.nim.

import std/[options, random]
import raylib, raymath
import camera, debug, dungeon, ecs, input, loot, resources, sprites,
       systems, tilemap
...
proc main =
  ...
  while not windowShouldClose():
    ...
    drawText("power: " & $swordPower, 10, 160, 20, Green)
